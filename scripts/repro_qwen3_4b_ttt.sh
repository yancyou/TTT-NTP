#!/usr/bin/env bash
# TTT-NTP on Qwen3-4B-Base: train (configs/pretrain/qwen3_4b.yaml, 1000 steps), export the
# checkpoint to HuggingFace layout, and run the closed-form RULER Full-13 evaluation.
#
# Usage: BASE=/path/to/Qwen3-4B-Base DATA=/path/to/long-data-collections OUT=/path/out \
#        bash scripts/repro_qwen3_4b_ttt.sh
set -euo pipefail

BASE=${BASE:?path to Qwen3-4B-Base checkpoint}
...
F='{"ttt_layers":[0,6,12,18,24,30],"ttt_mode":true,"ttt_proj":true,"ttt_proj_init":"diag_shared","ttt_conv":false,"ttt_predict_mode":"next","ttt_target":"hidden_states","ttt_chunk":1024,"ttt_norm_preserve":false,"ttt_inner_opt":"specnorm","ttt_lr":3.0}'

# ---- 1. train ----
mkdir -p "$OUT"
bash train.sh tasks/train_torch.py configs/pretrain/qwen3_4b.yaml \
  ...
  --train.max_steps "$STEPS" --train.save_steps 250 --train.save_epochs 0 \
  --train.global_batch_size 64 --train.micro_batch_size 2 \
  --train.lr 5.0e-6 --train.lr_warmup_ratio 0.02 --train.seed "$SEED" \
...
CF_NORM_PRESERVE=1 CKPT="$D/hf_ckpt" MODEL_ABBR=qwen3-4b-ttt OUT_DIR="$OUT/results" \
  LENGTHS="4k 8k 16k 32k" NUM_SAMPLES=100 \
  BATCH_4k=16 BATCH_8k=8 BATCH_16k=4 BATCH_32k=2 \
