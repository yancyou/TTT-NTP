#!/usr/bin/env bash
# TTT-NTP on Llama-3.1-8B: train (configs/pretrain/llama3_8b.yaml, 200 steps), export the
# checkpoint to HuggingFace layout, and run the closed-form RULER Full-13 evaluation.
#
# Usage: BASE=/path/to/Llama-3.1-8B DATA=/path/to/long-data-collections OUT=/path/out \
#        bash scripts/repro_llama_ttt.sh
set -euo pipefail

BASE=${BASE:?path to Llama-3.1-8B base checkpoint}
...
F='{"ttt_layers":[0,6,12,18,24,30],"ttt_mode":true,"ttt_proj":true,"ttt_proj_init":"small","ttt_conv":false,"ttt_predict_mode":"next","ttt_target":"hidden_states","ttt_chunk":1024,"ttt_norm_preserve":true,"ttt_inner_opt":"specnorm","ttt_lr":0.3}'

# ---- 1. train ----
mkdir -p "$OUT"
bash train.sh tasks/train_torch.py configs/pretrain/llama3_8b.yaml \
  ...
  --train.max_steps "$STEPS" --train.save_steps 100 --train.save_epochs 0 \
...
CF_NORM_PRESERVE=1 CKPT="$D/hf_ckpt" MODEL_ABBR=llama-ttt OUT_DIR="$OUT/results" \
