...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x, t: Optional[torch.Tensor] = None):
        ...
        _h_src = h_padded[:, :-1, :-1, :]   # drop last chunk, last token per chunk
        _t_tgt = t[:, :-1, 1:, :]           # drop last chunk, first token per chunk
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if not ttt_disable and target_states is None and self.is_ttt_layer:
            target_states = hidden_states
        mlp_t = None if ttt_disable else target_states
        hidden_states = self.mlp(hidden_states, t=mlp_t)
        hidden_states = residual + hidden_states
        return hidden_states
